...
get_keypress() {
  ...
  if [[ $k == $'\x1b' ]]; then
    local b; local -i i=0
    while IFS= read -rsk1 -t 0.01 b; do rest+="$b"; (( ++i > 4 )) && break; done
    seq="$k$rest"
    case "$seq" in
...
menu_single_select() {
  ...
  while true; do
    ...
    for (( i=1; i<=${#items[@]}; i++ )); do
      if [[ $i -eq $index ]]; then print -P "  ${BOLD}> ${items[$i]}${RESET}"
      else                           print -P "    ${items[$i]}"; fi
    done
...
menu_multi_select() {
  ...
  while true; do
    ...
    for (( i=1; i<=${#items[@]}; i++ )); do
      ...
      if [[ $i -eq $index ]]; then print -P "  ${BOLD}> $checked ${items[$i]}${RESET}"
      else                           print -P "    $checked ${items[$i]}"; fi
    done
    local proceed_row=$(( ${#items[@]} + 1 ))
    if [[ $index -eq $proceed_row ]]; then print -P "  ${BOLD}> Proceed${RESET}"
    else                                     print -P "    Proceed"; fi
...
if (( ${#SELECTED_INDEXES[@]} )); then
  print "Tasks chosen:"
  for idx in "${SELECTED_INDEXES[@]}"; do print " - ${menu_items[$idx]}"; done
else
...
if (( ${#TODO_ITEMS_MSGS[@]} )); then
  ...
  if [[ "$REPLY" -eq 2 ]]; then
    typeset -A dedup; local id; for id in "${TODO_ITEMS_TASKS[@]}"; do dedup["$id"]=1; done
    typeset -a issue_ids issue_labels; for id _ in "${(@kv)dedup}"; do issue_ids+=("$id"); issue_labels+=("${TASK_LABEL[$id]:-$id}"); done
    if (( ${#issue_ids[@]} )); then
      menu_multi_select issue_labels
      if (( ${#SELECTED_INDEXES[@]} )); then
        for idx in "${SELECTED_INDEXES[@]}"; do
          id="${issue_ids[$idx]}"; [[ -z "${TASK_FN[$id]-}" ]] && { warn "No direct handler for: $id"; continue; }
